...
import {
  ...
  saveMeetingMinutes,
  generateProposalPDF
} from '../../api/apiFunctions/User_Sales_exe/momApi';
...
function MeetingMinutesForm({ navigation: navProp, route, onClose, action }) {
  ...
  return (
    ...
    <Modal
      ...
      animationType='slide'>
      <SafeAreaView style={{ flex: 2, backgroundColor: '#13132a' }}>
        <KeyboardAvoidingView
          behavior={Platform.OS === "ios" ? "padding" : undefined}
          style={{ flex: 3, justifyContent: 'center' }}
        >
          <View style={{ flex: 4, backgroundColor: '#13132a' }}>
            {/* Header Row: Only cross (close) and back button */}

            <View style={[styles.headerRow, { marginTop: Platform.OS == 'ios' ? '50' : '' }]}>
