...
export default function VisitFormScreen() {
    ...
    (async () => {
      ...
      try {
        const res = await fetch(
          `${BASE_URL}/salesperson/getRoleListing`,
      ...
      try {
        const res = await fetch(
          `${BASE_URL}/saleStage/getSaleStage?productId=${prodId}&customerID=${custId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const json = await res.json();
        setStages(json.data || []);
 ...
 const hasPermission = await requestCameraPermission();
  ...
  const submitVisit = async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api/v1/salesperson/uploadVisit`, { 
        method: 'POST',
